...
public class GauzatuEragiketaMockWhiteTest {
	...
	public void test1() {
		...
		try {
			...
			ArgumentCaptor<Driver> driverCaptor =
					...
					.merge(driverCaptor.capture());
					//verify parameter values as usual using JUnit asserts
					assertEquals(username, ((Driver)driverCaptor.getValue()).getUsername());
			assertTrue(emaitza);
		} catch(Exception e) {
			e.printStackTrace();
			fail();
	...
	public void test5() {
		...
		try {
            ...
            when(db.createQuery("SELECT u FROM User u WHERE u.username = :username", User.class)).thenThrow(RuntimeException.class);
